...
_VALID_CATEGORIES = frozenset({"weather", "sports", "politics", "finance"})
...
async def _call_prediction_api(
    ...
    if len(title) < 10:
        title = f"Will {title} resolve as Yes?"
    category = entry.metadata.get("category", "finance")
    ...
    try:
        ...
        try:
            risk_data = await _call_risk_manager(
                http, settings,
                prediction_id, probability, confidence, ev, ev,
        ...
        if approved:
            if settings.dry_run:
                ...
                price = int(recommended_price) if recommended_price else max(1, int(probability * 100))
